...
## 自律開発モード（Multi-layer Quality Control）

### 概要
PMは `task.md` の `自律: YES` タスクを、人間の指示なしに自律的に実行できる。
人間は非同期で `reports/` の報告とgit logを確認し、方針修正や新タスク追加を行う。

品質は4層のレイヤで担保する:

| レイヤ | 役割 | 実体 |
|---|---|---|
| 自律設計レイヤ | ReActループでタスクを自走 | PMの自律サイクル（下記） |
| ガーディアンレイヤ | メタ認知チェック（根拠・一貫性・完了条件充足） | `.claude/agents/guardian.md` |
| 知識レイヤ | 現場の作法・ドメイン知識を注入 | `docs/patterns.md`, `docs/baseball-knowledge.md` |
| 情報アクセスレイヤ | コードベース・外部情報の効率的な取得 | CLAUDE.md, Explore agent, WebSearch |
...
| 判断カテゴリ | 自律OK | 人間確認必要 |
|---|---|---|
| task.mdに`自律: YES`で完了条件付きのタスク | YES | - |
| バグ修正（既存動作の回帰） | YES | - |
| バランス調整（NPB準拠範囲が明確） | YES | - |
| GAME_SPEC.md / CLAUDE.md の実態同期 | YES | - |
| 新しいUIページの追加 | - | YES |
| モデル型の大幅変更（5フィールド以上追加） | - | YES |
| ゲームルールの新規追加 | - | YES |
| ゲームデザイン判断（「何が面白いか」） | - | YES |
| 既存UIのレイアウト大幅変更 | - | YES |

### 暴走防止ルール
1. **完了条件のないタスクは着手しない**: 何をもって「終わり」かが不明なタスクはスキップ
2. **迷ったらスキップ**: 設計判断に迷った場合、そのタスクを飛ばして次へ進む
3. **コミットは小さく**: 1タスク1コミットが基本。巨大な変更にならないよう分割
4. **ループ禁止**: ビルドエラーやテスト失敗で3回リトライしても解決しない場合は報告して中断
5. **型の大幅変更時は慎重に**: `models/player.ts` や `store/game-store.ts` を触る場合はセーブ互換を必ず確認
6. **ガーディアン差し戻しは2回まで**: 同じタスクで2回FAILしたら人間に報告して中断
