...
│   ├── fielding-agent.ts  # エージェント守備（ThrowPlay構築・ベースカバー）
│   ├── fielding-agent-types.ts # 守備エージェント型定義（ThrowPlay等）
...
### 自律パイプライン（バックグラウンド実行）
PMは `自律: YES` のタスクを以下のパイプラインで自動実行する:
```
1. PM: TaskCreate で進捗表示（日本語）
2. PM → impl-* (run_in_background) で実装委譲
3. 実装完了通知 → PM が自動で tester (run_in_background) 起動
4. テスト完了通知 → PM が自動で reviewer (run_in_background) 起動
5. レビュー完了通知 → PM がコミット＆push → ユーザーに結果報告
```
- パイプライン中もPMは会話可能（ブロックされない）
- 各ステップで問題があれば即座にユーザーに報告して中断
- 大きい変更（シミュレーション変更等）はレビュー後に guardian も追加
