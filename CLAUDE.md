...
- **弾道(trajectory)**: BatterAbilitiesに弾道(1-4)を追加。パワー相関で生成（`1 + (power/100)*2.5 ± 0.75`）。シミュレーションでは`angleMean += (trajectory-2)*3`で打球角度に影響（弾道1=-3°, 2=±0°, 3=+3°, 4=+6°）。既存セーブ互換は`?? 2`でデフォルト。
