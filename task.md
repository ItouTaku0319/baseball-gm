...
- 現状の問題（コード調査済み）:
  - 能力値テーブル:
    - 「走力: 三塁打率に影響（盗塁は未実装）」→ 盗塁はsimulation.tsで実装済み
    - 「守備力: 現在シミュレーション未使用」→ fielding-ai.tsで到達速度・捕球率に使用中
    - 「スタミナ: 現在シミュレーション未使用（完投固定）」→ STAMINA_PER_PITCH=0.7で消費、50%以下で疲労ペナルティ
    - 「精神力: 現在シミュレーション未使用」→ **実際に未使用のまま**（GAME-007で活用予定）
  - 未実装機能テーブル（6項目が陳腐化）:
    - 「盗塁: シミュレーション未使用」→ 実装済み
    - 「セーブ: savePitcherId常にnull」→ セーブ判定実装済み
    - 「ホールド: インクリメントなし」→ ホールド判定実装済み
    - 「守備力: シミュレーション未使用」→ fielding-ai.tsで使用中
    - 「打順設定: 手動設定不可」→ 打順UIで手動設定可能
    - 「プレーオフ: スケジュール生成なし」→ CS+日本シリーズ完全実装済み
  - シーズンフェーズ: 「playoffs: 未実装」→ 実装済み
- 一部対応済み（GAME-008の過程で更新された項目）:
  - [x] 投手起用方針セクション追加済み（先発6種・リリーフ5種・maxInnings・連投制限）
  - [x] リリーフ投手: 「実装済み」に更新済み
- 残り完了条件:
  - [ ] 能力値テーブルの陳腐化記述を実コードに合わせて更新
  - [ ] 未実装機能テーブルの6項目を修正（削除 or 実装済みに更新）
  - [ ] シーズンフェーズの「playoffs: 未実装」を修正
