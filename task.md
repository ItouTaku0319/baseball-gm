...
### Sprint 1 (2/26-2/28): 守備AIエージェント型 + 打球生成設計

| # | タスク | 担当 | 状態 | 完了条件 |
|---|--------|------|------|---------|
| 1 | エージェント型守備AI実装 | impl-engine (別タブ) | 進行中 | plan.txt通りの実装、グリッドテスト全ゲート通過、バランステスト通過 |
| 2 | 打球生成ロジック再設計 | 別タブ | 設計中 | docs/batted-ball-design-context.md ベースの設計書完成 |
| 3 | 守備テスト拡充（野手視点テスト） | PM | 設計完了 | docs/fielding-test-guide.md 作成済み。実装はSprint 2 |

### Sprint 1 の方針
- 守備AIの実装は別タブに委譲済み（plan.txt + 完了条件）
- 打球生成は設計のみ（実装は守備AI完成後）
- テストガイドは作成済み、実装エージェントが参照できる状態
