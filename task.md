...
- [x] 投手起用法: 1球ずつ打席シミュ、スタミナ消費・疲労低下、イニング間/途中交代AI、役割別リリーフ(守護神/SU/中継ぎ/敗戦処理)、セーブ/ホールド判定、起用方針UI
- [x] 投手起用方針の個別設定: 先発6種(完投/勝利投手/調子次第/スタミナ温存/オープナー/ショートスターター)、リリーフ5種(守護神/リード時/接戦時/ビハインドOK/敗戦処理)、maxInnings制限、投手枠(先発MAX6+リリーフMAX8)
...
### Step 9: 守備AI (fielding-ai.ts) ✅ 完了
- [x] 2Dフィールド座標系とデフォルト守備位置の定義
- [x] 打球着地座標計算 (calcBallLanding: ゴロ=摩擦減速、フライ=放物運動+空気抵抗)
- [x] 9野手の到達時間計算 (evaluateFielders: 反応時間+走行距離、ライナー遅延あり)
- [x] 物理ベースの結果判定 (resolvePlayWithAI: 捕球→送球→走者との競争)
- [x] ゴロ: 捕球後の送球vs走者タイミングで内野安打/アウトが自然に決定
- [x] フライ/ライナー: 到達+捕球試行、不到達→回収+送球で長打判定
- [x] 旧ロジック完全置き換え (determineFielderFromBall/resolveInPlayFromBall/determineHitType削除)
- [x] 1000試合バランステストフレームワーク (scripts/test-balance-full.ts) 追加
- [x] 検証結果: AVG.254, HR/試合1.37, K%18.5%, BB%7.6%, BABIP.302, GO/AO1.08 (全指標正常、異常パターン0件)
...
## 投球分析画面 ✅ 完了
- [x] PitchTypeに`fastball`(ストレート)追加
- [x] AtBatLogに球種(pitchType)・コース(pitchLocation)追加
- [x] selectPitch: 決め球選択（球速/レベル重み付き、結果で調整）
- [x] generatePitchLocation: コース生成（制球で分散制御、結果別傾向）
- [x] 投球分析ページ新規作成（ストライクゾーンSVG・球種別成績・コースヒートマップ・打席ログ）
- [x] ダッシュボードにナビリンク追加
...
### ~~dragFactor定数の一元化~~ ✅ 完了
`physics-constants.ts`を新規作成し、3ファイルのマジックナンバーを定数importに統一。`estimateDistance`と`calcBallLanding`の飛距離計算式も統一済み。
...
### ~~HR率のバランス検証~~ ✅ 守備AI実装で解決済み
守備AI(fielding-ai.ts)の導入とdragFactor=0.61への調整で、1000試合テストで全指標NPB範囲内を確認済み。

### 打球軌道ポップアップに守備側選手情報を表示
現在の打球軌道ポップアップは打球のみの表示。将来的に以下の守備側情報を追加したい:
- 守備位置に選手の丸アイコン（○）＋名前を表示
- 守備力・肩力・捕球の能力値をツールチップまたはカードで表示
- 送球アニメーション（捕球→送球→塁カバー）
- 前提: AtBatLogに fielderPosition があるため、各打席でどの選手がどの位置にいたかは取得可能。選手の能力値はロスターから逆引き可能。
