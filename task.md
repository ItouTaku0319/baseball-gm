...
（未計画 — `/sprint plan` で次のスプリントを計画する）

### 過去のスプリント結果
- Sprint 1 (2/26-2/28): エージェント型守備AI実装完了、本番切替完了、ThrowPlayチェーン導入完了
- Sprint 2 (2/27-3/1): 守備AI品質改善ロードマップ — 全タスク未着手のまま超過→バックログに戻し
...
- 自律: YES
- コスト: 中
- 概要: GC圧力削減、calcPathInterceptキャッシュ、vec2距離二乗比較、知覚間引き
- 完了条件: 1000試合テストの実行時間を計測し改善前比で高速化

#### BL-E02: 打球分布検証テスト
- 優先度: HIGH
- 自律: YES
- コスト: 低
- 完了条件: GB%/FB%/LD%/PU%・方向分布の自動検証テスト

#### BL-E03: 投手守備機会改善
- 優先度: HIGH
- 自律: YES
- コスト: 低
- 完了条件: P TC/G >= 1.0 (現状0.13)
...
- 自律: YES
- コスト: 中
- 完了条件: SS TC/G < 5.0、全ポジションTC/GがNPBの70-130%範囲内

#### BL-E06: ゴールデンテスト作成(30-50件)
- 優先度: MEDIUM
- 自律: NO
- 完了条件: 代表的打球パターンの期待結果を定義、全ケース通過
...
#### BL-K01: 投手守備機会が少ない
- バント実装済みだが頻度がまだ低い可能性あり（BL-E03と関連）

#### BL-K02: フライボール安打率がやや高め(~20%)
- NPB標準(~15%)より高い。外野手デフォルト位置調整で改善可能
