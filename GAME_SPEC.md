...
打席前に走者が盗塁を試みる。走力 vs 捕手の肩力で成否が決まる。
...
**2アウト時の係数**: `SB_TWO_OUTS_FACTOR = 1.3`（2アウト時はアウトになってもチェンジになるだけのためリスクが低く、試行率を30%増し）

#### 成功率計算式

```
successRate = clamp(BASE + (speed - 50) × SPEED_BONUS - (arm - 50) × ARM_PENALTY, MIN, MAX)
```

| パラメータ | 2塁盗塁 | 3塁盗塁 |
|---|---|---|
| BASE | 0.60 | 0.55 |
| SPEED_BONUS（speed 1ptあたり） | 0.005 | 0.005 |
| ARM_PENALTY（捕手arm 1ptあたり） | 0.004 | 0.005 |
| MIN | 0.25 | 0.25 |
| MAX | 0.90 | 0.90 |

speed=50、捕手arm=50 が基準点（補正なし）。
...
打席前（盗塁判定後）に1回判定。走者がいない場合は発生しない。

#### 発生確率（1打席あたり）

| 種類 | 計算式 | 基準値(control/catching=50) |
|---|---|---|
| WP | `WP_BASE_RATE + (50 - pitcher.control) × WP_CONTROL_FACTOR` | 0.0008/打席 |
| PB | `PB_BASE_RATE + (50 - catcher.catching) × PB_CATCHING_FACTOR` | 0.0003/打席 |

定数値:
- `WP_BASE_RATE = 0.0008`
- `WP_CONTROL_FACTOR = 0.000012`（control1ポイントあたりの発生率変化）
- `PB_BASE_RATE = 0.0003`
- `PB_CATCHING_FACTOR = 0.000004`（catching1ポイントあたりの発生率変化）
...
- 3塁走者はホームイン（得点）
- **WP**: 投手のミス → 自責点に加算
- **PB**: 捕手のミス → 非自責点（`pitcherLog.earnedRuns`に加算しない）
