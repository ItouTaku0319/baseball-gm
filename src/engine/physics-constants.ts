...
export const POPUP_ANGLE_START = 40;  // EV減衰開始角度
export const POPUP_ANGLE_FULL = 55;   // 完全キャップ角度（これ以上はPOPUP_EV_CAPに収束）
export const POPUP_EV_CLASSIFY_THRESHOLD = 95; // この速度以下の40-49°打球はpopup分類（着地距離≈40m以下）
...
export const GROUND_BALL_CATCH_FLOOR = 0.70;          // 捕球率の下限
...
// 反応時間（ポジション別）
// 内野手: 実測0.2-0.4s → 0.30s、捕手: 正面視0.45s、外野手/投手: 0.50s
export const AGENT_REACTION_INFIELD = 0.30;      // 内野手(3B-SS)基本反応時間(秒)
export const AGENT_REACTION_CATCHER = 0.45;      // 捕手基本反応時間(秒)
export const AGENT_REACTION_DEFAULT = 0.50;       // 外野手・投手基本反応時間(秒)
...
export const AGENT_BASE_SPEED = 7.2;             // 全ポジション共通基本走速(m/s) skill=50で8.2m/s≈MLB平均27ft/s
export const AGENT_SPEED_SKILL_FACTOR = 2.0;     // speed/100 あたりの走速追加(m/s)
...
export const RUNNER_SPEED_BASE = 5.8;            // 走者の基本走速(m/s) — speed0→4.7s, speed50→4.1s, speed100→3.6s (NPB準拠)
export const RUNNER_SPEED_SCALE = 1.8;           // speed/100 あたりの走速追加(m/s)
...
export const THROW_ERROR_SKILL_SCALE = 0.03;     // fielding/100 あたりのエラー率低減
export const THROW_ERROR_DISTANCE_SCALE = 0.03;  // 送球距離(m)/100 あたりのエラー率増加
...
export const RETRIEVER_PICKUP_TIME = 0.5;        // ボール拾い上げ+送球体勢までの時間(秒)
export const RETRIEVER_PICKUP_RADIUS = 1.5;     // ボール拾い上げ可能距離(m)
export const GROUND_BALL_APPROACH_WAIT_DIST = 10; // 停止ゴロの捕球待ち距離(m): この距離以内の野手がいればpre-catch継続
...
// ballType === "popup" → launchAngle >= 50°、ballType === "line_drive" → launchAngle < 25°
export const POPUP_LAUNCH_ANGLE = 50;     // この角度以上 → popout
export const LINER_LAUNCH_ANGLE_MAX = 25; // この角度未満 → lineout (MLB実データ: ライナー10-25度)
